...
export const AuthProvider = ({ children }) => {
  ...
  // ============================================================================
  // DEMO BYPASS FUNCTIONS - Complete Set for Manual Step Control
  // ============================================================================
  
  // Individual bypass setter functions
  const setEmailVerified = useCallback((verified) => {
    console.log('Demo bypass: Setting email verified to', verified);
    dispatch({ type: 'SET_EMAIL_VERIFIED', payload: verified });
    if (verified && !state.email) {
      dispatch({ type: 'SET_EMAIL', payload: 'demo@example.com' });
    }
  }, [state.email]);

  const setPhoneVerified = useCallback((verified) => {
    console.log('Demo bypass: Setting phone verified to', verified);
    dispatch({ type: 'SET_PHONE_VERIFIED', payload: verified });
    if (verified && !state.phone) {
      dispatch({ type: 'SET_PHONE', payload: '+1234567890' });
    }
  }, [state.phone]);

  const setBiometricVerified = useCallback((verified) => {
    console.log('Demo bypass: Setting biometric verified to', verified);
    dispatch({ type: 'SET_BIOMETRIC_VERIFIED', payload: verified });
    if (verified) {
      dispatch({ type: 'SET_FALLBACK_SETUP_COMPLETE', payload: true });
    }
  }, []);

  const setProfileCreated = useCallback((created) => {
    console.log('Demo bypass: Setting profile created to', created);
    dispatch({ type: 'SET_PROFILE_CREATED', payload: created });
    if (created && !state.userData) {
      const demoUserData = {
        id: 'demo-user-123',
        sngine_email: state.email || 'demo@example.com',
        sngine_phone: state.phone || '+1234567890',
        user_type: 'Individual Election Creators',
        admin_role: 'user',
        subscription_status: 'free',
        name: 'Demo User',
        created_at: new Date().toISOString()
      };
      dispatch({ type: 'SET_USER_DATA', payload: demoUserData });
      dispatch({ type: 'SET_USER_ID', payload: demoUserData.id });
      localStorage.setItem('vottery_user_data', JSON.stringify(demoUserData));
    }
  }, [state.email, state.phone, state.userData]);

  const setSecurityQuestionsSetup = useCallback((setup) => {
    console.log('Demo bypass: Setting security questions setup to', setup);
    dispatch({ type: 'SET_SECURITY_QUESTIONS_SETUP', payload: setup });
    if (setup) {
      const demoQuestions = [
      ...
      dispatch({ type: 'SET_COLLECTED_QUESTIONS', payload: demoQuestions });
    }
  }, []);

  const setCurrentStep = useCallback((step) => {
    console.log('Demo bypass: Setting current step to', step);
    dispatch({ type: 'SET_STEP', payload: step });
  }, []);

  const setIsAuthenticated = useCallback((authenticated) => {
    console.log('Demo bypass: Setting authenticated to', authenticated);
    dispatch({ type: 'SET_AUTHENTICATED', payload: authenticated });
    if (authenticated) {
      const demoTokens = {
        accessToken: 'demo-access-token-' + Date.now(),
        refreshToken: 'demo-refresh-token-' + Date.now(),
        tokenExpiry: { accessTokenExpiry: '7d', refreshTokenExpiry: '30d' }
      };
      TokenManager.setTokens(demoTokens.accessToken, demoTokens.refreshToken, demoTokens.tokenExpiry);
      
      const sessionData = {
        userId: state.userData?.id || 'demo-user-123',
        email: state.email || 'demo@example.com',
        phone: state.phone || '+1234567890',
        userType: 'Individual Election Creators',
        adminRole: 'user',
      ...
      toast.success('Demo bypass: Authentication completed!');
    }
  }, [state.userData, state.email, state.phone, createSession]);

  // Master bypass function
  const bypassAllSteps = useCallback(() => {
    console.log('Demo bypass: Completing all authentication steps');
    ...
    dispatch({ type: 'SET_USER_ID', payload: 'demo-user-123' });
    ...
    const sessionData = {
      ...
      email: demoUserData.sngine_email,
      phone: demoUserData.sngine_phone,
      userType: demoUserData.user_type,
      adminRole: demoUserData.admin_role,
      subscriptionStatus: demoUserData.subscription_status,
      authenticated: true,
      profileCreated: true,
      timestamp: Date.now()
    };
    ...
    const permissions = derivePermissionsFromRole(demoUserData.admin_role);
    setRolePermissions({
      admin_role: demoUserData.admin_role,
      user_type: demoUserData.user_type,
      subscription_status: demoUserData.subscription_status,
      permissions: permissions,
      userData: demoUserData
    });
    
    toast.success('Demo bypass: All steps completed! Welcome to Vottery!');
  }, [state.email, state.phone, createSession]);

  // ============================================================================
  // END OF BYPASS FUNCTIONS
  // ============================================================================
  ...
  // Context value with all functions including bypass functionality
  const value = {
    ...state,
    dispatch,
...
//this is the last workable file. just to add bypass i am adding above file
// import React, { createContext, useContext, useReducer, useCallback, useEffect, useState, useRef } from 'react';
// import { useSecurity } from './SecurityContext';
// import { SecurityUtils } from '../utils/security';
// import { toast } from 'react-hot-toast';
