...
const LotteryVotingDemo = ({ election, onClose }) => {
  ...
  const [prizeAmount] = useState(election?.reward_amount ? parseFloat(election.reward_amount) : 50000);
  const [winnerCount] = useState(election?.winner_count || 3);
  const [totalVotes, setTotalVotes] = useState(1247);
  const [isLotterized] = useState(election?.is_lotterized !== false);
  ...
  const electionData = {
    ...
    options: election?.questions?.[0]?.answers?.map((answer, index) => ({
      ...
      image: answer.imageUrl || `https://picsum.photos/400/300?random=${index + 1}`,
      color: ['#EF4444', '#10B981', '#F59E0B', '#8B5CF6', '#3B82F6'][index % 5]
    })) || [
      { id: 1, name: "iPhone 16 Pro Max", votes: 423, color: '#EF4444', image: "https://picsum.photos/400/300?random=1" },
      { id: 2, name: "Samsung Galaxy S25", votes: 387, color: '#10B981', image: "https://picsum.photos/400/300?random=2" },
      { id: 3, name: "Google Pixel 9 Pro", votes: 298, color: '#F59E0B', image: "https://picsum.photos/400/300?random=3" },
      { id: 4, name: "OnePlus 13 Pro", votes: 139, color: '#8B5CF6', image: "https://picsum.photos/400/300?random=4" }
    ]
  };
  ...
  useEffect(() => {
    ...
    const initialBalls = Array.from({ length: 12 }, (_, i) => ({
      id: `VT-${String(i + 1).padStart(3, '0')}`,
      color: colors[i % colors.length],
      number: String(1000 + i)
    }));
    setBallsInMachine(initialBalls);
  }, []);

  // Rotation animation
  useEffect(() => {
    const interval = setInterval(() => {
      setMachineRotation(prev => (prev + 1) % 360);
    }, 50);
  ...
  const handleVoteSubmit = () => {
    ...
    // First phase: ballot flies to machine (1000ms)
    setTimeout(() => {
      // Second phase: ballot enters and transforms (500ms)
      setTimeout(() => {
        const newBall = {
          id: userVoteId,
          color: '#FFD700',
        ...
        // Third phase: show confirmation (500ms delay)
        setTimeout(() => {
          setCurrentStep(2);
        }, 500);
      }, 500);
            ...
            >
              ...
              <div className="absolute inset-4 rounded-full bg-gradient-to-br from-transparent via-white/10 to-transparent">
                ...
                {ballsInMachine.map((ball, index) => {
                  ...
                  const isNewBall = ball.isNew && Date.now() - (ball.addedTime || 0) < 2000;
                  const scale = isWinner ? 2.5 : isNewBall ? 1.5 : 1 + Math.sin(time + index) * 0.1;
                  ...
                  return (
                    <div
                      ...
                      } ${isNewBall ? 'animate-bounce' : ''}`}
                      style={{
                        width: isWinner ? '48px' : isNewBall ? '40px' : '32px',
                        height: isWinner ? '48px' : isNewBall ? '40px' : '32px',
                        left: `calc(50% + ${x}px)`,
                        top: `calc(50% + ${y}px)`,
                        background: isWinner 
                        ...
                        boxShadow: isWinner 
                          ...
                          : isNewBall
                          ? '0 0 25px #FFD700, 0 8px 20px #FFD70099, inset 0 0 15px rgba(255,255,255,0.5)'
                          : `0 8px 20px ${ball.color}66, inset 0 0 10px rgba(255,255,255,0.3)`,
                        zIndex: isWinner ? 100 : isNewBall ? 50 : Math.floor(50 + z),
                        transformStyle: 'preserve-3d'
                      }}
                    >
                      {/* Ball shine effect */}
                      ...
                      {/* New ball badge */}
                      {isNewBall && (
                        <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-2 py-1 rounded text-xs font-bold whitespace-nowrap shadow-lg animate-pulse">
                          NEW!
                        </div>
                      )}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[95vh] overflow-y-auto">
          ...
          <div className="mt-4 bg-white/20 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-lg">${prizeAmount.toLocaleString()}</span>
              </div>
              <div className="text-right">
                <div className="text-sm opacity-90">Winners</div>
                <div className="font-bold">{winnerCount}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          ...
          {currentStep === 1 && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                  ...
                  {electionData.options.map((option) => (
                    <button
                      ...
                      className={`p-4 border-2 rounded-xl transition-all hover:shadow-lg ${
                        selectedOption?.id === option.id ? 'border-purple-500 bg-purple-50 scale-105' : 'border-gray-200'
                      }`}
                    >
                      <img 
                        src={option.image} 
                ...
                <div className="text-center">
                  <button
                    ...
                    className="bg-purple-600 text-white px-8 py-3 rounded-lg font-bold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-purple-700 flex items-center gap-2 mx-auto"
                  >
                    <SparklesIcon className="h-5 w-5" />
                    {selectedOption ? `Vote for ${selectedOption.name}` : 'Select an option first'}
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center">
                {isLotterized && <GlassLotteryMachine />}
              </div>
            </div>
          )}
          ...
          {currentStep === 3 && (
            <div className="space-y-6 text-center">
              <h3 className="text-2xl font-bold">🎰 Lottery Draw in Progress</h3>
              <GlassLotteryMachine />
              {lotteryRunning && (
          ...
          {currentStep === 4 && winner && (
            <div className="space-y-6 text-center">
              <div className="text-6xl">🏆</div>
              <h3 className="text-3xl font-bold text-yellow-600">Winner Selected!</h3>
              ...
              <div className="bg-gradient-to-r from-yellow-400 to-orange-500 text-white rounded-2xl p-8 max-w-md mx-auto">
                ...
                <div className="bg-white/20 rounded-lg p-4 mt-4">
                  <div className="text-3xl font-bold">${prizeAmount.toLocaleString()}</div>
      ...
      {voteFlying && (
        <>
          {/* Stage 1: Ballot flying from left side to machine top */}
          <div 
            className="fixed z-50"
            style={{
              left: voteFlying ? '50%' : '15%',
              top: voteFlying ? '25%' : '60%',
              transform: `translate(-50%, -50%) rotate(${voteFlying ? 720 : 0}deg) scale(${voteFlying ? 0.4 : 1})`,
          ...
          >
            <div className="relative">
              {/* Ballot Paper */}
              <div className="w-24 h-32 bg-white rounded-lg shadow-2xl border-2 border-purple-500 p-2 relative">
                <div className="text-xs font-bold text-center text-purple-600 mb-1">BALLOT</div>
                <div className="w-full h-20 bg-gradient-to-r from-purple-500 to-blue-500 rounded flex flex-col items-center justify-center p-1">
                  <span className="text-white font-bold text-xs mb-1">VOTE</span>
                  <span className="text-white text-xs">{selectedOption?.name?.substring(0, 10)}</span>
                </div>
                <div className="text-center mt-1">
                  <span className="text-xs font-mono text-gray-600">{userVoteId.substring(0, 10)}</span>
                </div>
              </div>
              
              {/* Motion trail effect */}
              <div className="absolute inset-0 bg-gradient-to-r from-purple-400 to-blue-400 rounded-lg blur-xl opacity-50 -z-10"></div>
            </div>
          </div>

          {/* Stage 2: Transformation sparkles at the top opening */}
          <div 
            className="fixed z-50"
            style={{
              left: '50%',
          ...
          >
            ...
            <div className="relative w-16 h-16">
              <div className="absolute inset-0 animate-ping">
                <div className="w-full h-full rounded-full bg-yellow-400 opacity-75"></div>
              </div>
